...
void init_combiner()
{
    ...
    // bw shader
// 		fragment_bw_shader_object = glCreateShaderObjectARB(GL_FRAGMENT_SHADER_ARB);

// 		fragment_shader = (char*)malloc(strlen(fragment_shader_header)+
// 		 						 strlen(fragment_shader_bw)+
// 								 strlen(fragment_shader_end)+1);
// 		strcpy(fragment_shader, fragment_shader_header);
// 		strcat(fragment_shader, fragment_shader_bw);
// 		strcat(fragment_shader, fragment_shader_end);
// 		glShaderSourceARB(fragment_bw_shader_object, 1, (const GLcharARB**)&fragment_shader, NULL);
// 		free(fragment_shader);

// 		glCompileShaderARB(fragment_bw_shader_object);
    ...
    // bw program
// 		program_object = glCreateProgramObjectARB();
// 		program_object_bw = program_object;
// 		glAttachObjectARB(program_object, fragment_bw_shader_object);
// 		glAttachObjectARB(program_object, vertex_shader_object);
// 		glLinkProgramARB(program_object);
// 		glUseProgramObjectARB(program_object);
...
void free_combiners()
{
	if(glsl_support)
	{
		...
		/*glDeleteObjectARB(program_object_default);
		glDeleteObjectARB(fragment_shader_object);

		for(i=0; i<number_of_programs; i++)
		{
			glDeleteObjectARB(shader_programs[i].program_object);
			glDeleteObjectARB(shader_programs[i].fragment_shader_object);
		}*/
...
// 	glUseProgramObjectARB(program_object_bw);
// 	texture0_location = glGetUniformLocationARB(program_object, "texture0");
// 	glUniform1iARB(texture0_location, 0);
// }
               ...
               FxBool invert )
{
	...
	else
	{
		...
		fct[num_tex+1] = GL_REPLACE;
		source0[num_tex+1] = GL_PREVIOUS_ARB;
		operand0[num_tex+1] = GL_SRC_COLOR;
		
		switch(function)
		{
		case GR_COMBINE_FUNCTION_LOCAL:
			fct[num_tex] = GL_REPLACE;
			source0[num_tex] = setLocalColorSource(local);
			operand0[num_tex] = GL_SRC_COLOR;
			break;
		case GR_COMBINE_FUNCTION_SCALE_OTHER:
			switch(factor)
			{
			case GR_COMBINE_FACTOR_LOCAL:
				fct[num_tex] = GL_MODULATE;
				source0[num_tex] = setLocalColorSource(local);
				operand0[num_tex] = GL_SRC_COLOR;
				source1[num_tex] = setOtherColorSource(other);
				operand1[num_tex] = GL_SRC_COLOR;
				break;
			case GR_COMBINE_FACTOR_ONE:
				fct[num_tex] = GL_REPLACE;
				source0[num_tex] = setOtherColorSource(other);
				operand0[num_tex] = GL_SRC_COLOR;
				break;
			case GR_COMBINE_FACTOR_ONE_MINUS_LOCAL:
				fct[num_tex] = GL_MODULATE;
				source0[num_tex] = setLocalColorSource(local);
				operand0[num_tex] = GL_ONE_MINUS_SRC_COLOR;
				source1[num_tex] = setOtherColorSource(other);
				operand1[num_tex] = GL_SRC_COLOR;
				break;
			case GR_COMBINE_FACTOR_OTHER_ALPHA:
				fct[num_tex] = GL_MODULATE;
				source0[num_tex] = setOtherColorSource(other);
				operand0[num_tex] = GL_SRC_ALPHA;
				source1[num_tex] = setOtherColorSource(other);
				operand1[num_tex] = GL_SRC_COLOR;
				break;
			case GR_COMBINE_FACTOR_TEXTURE_RGB:
				fct[num_tex] = GL_MODULATE;
				source0[num_tex] = GL_PREVIOUS_ARB;
				operand0[num_tex] = GL_SRC_COLOR;
				source1[num_tex] = setOtherColorSource(other);
				operand1[num_tex] = GL_SRC_COLOR;
				break;
			default:
				display_warning("grColorCombine(GR_COMBINE_FUNCTION_SCALE_OTHER) : unknown factor : %x", factor);
			}
			break;
		case GR_COMBINE_FUNCTION_SCALE_OTHER_ADD_LOCAL:
			switch(factor)
			...
			case GR_COMBINE_FACTOR_TEXTURE_RGB:
				...
				source1[num_tex] = setOtherColorSource(other);
				operand1[num_tex] = GL_SRC_COLOR;
				fct[num_tex+1] = GL_ADD;
				source0[num_tex+1] = GL_PREVIOUS_ARB;
				operand0[num_tex+1] = GL_SRC_COLOR;
				source1[num_tex+1] = setLocalColorSource(local);
				operand1[num_tex+1] = GL_SRC_COLOR;
				if (source1[num_tex+1] == GL_PREVIOUS_ARB) source1[num_tex+1] = GL_TEXTURE0_ARB;
				break;
			case GR_COMBINE_FACTOR_ONE:
				fct[num_tex] = GL_ADD;
				source0[num_tex] = setOtherColorSource(other);
				operand0[num_tex] = GL_SRC_COLOR;
				source1[num_tex] = setLocalColorSource(local);
				operand1[num_tex] = GL_SRC_COLOR;
				break;
			case GR_COMBINE_FACTOR_LOCAL:
				fct[num_tex] = GL_MODULATE;
			...
			case GR_COMBINE_FACTOR_ONE_MINUS_LOCAL_ALPHA:
				fct[num_tex] = GL_MODULATE;
				source0[num_tex] = setLocalColorSource(local);
				operand0[num_tex] = GL_ONE_MINUS_SRC_ALPHA;
				source1[num_tex] = setOtherColorSource(other);
				operand1[num_tex] = GL_SRC_COLOR;
				fct[num_tex+1] = GL_ADD;
				source0[num_tex+1] = GL_PREVIOUS_ARB;
				...
				break;
			default:
				display_warning("grColorCombine(GR_COMBINE_FUNCTION_SCALE_OTHER_ADD_LOCAL) : unknown factor : %x", factor);
			}
			break;
		case GR_COMBINE_FUNCTION_SCALE_OTHER_MINUS_LOCAL:
			switch(factor)
			{
			case GR_COMBINE_FACTOR_TEXTURE_RGB:
				// this combiner mode isn't implemented correctly and doesn't work on a TNT
				fct[num_tex] = GL_SUBTRACT_ARB;
				source0[num_tex] = setOtherColorSource(other);
				operand0[num_tex] = GL_SRC_COLOR;
				source1[num_tex] = setLocalColorSource(local);
				operand1[num_tex] = GL_SRC_COLOR;
				fct[num_tex+1] = GL_MODULATE;
				source0[num_tex+1] = GL_TEXTURE0_ARB;
				operand0[num_tex+1] = GL_SRC_COLOR;
				source1[num_tex+1] = GL_PREVIOUS_ARB;
				operand1[num_tex+1] = GL_SRC_COLOR;
				break;
			case GR_COMBINE_FACTOR_ONE:
				// doesn't work on TNT
				...
				break;
			default:
				display_warning("grColorCombine(GR_COMBINE_FUNCTION_SCALE_OTHER_MINUS_LOCAL) : unknown factor : %x", factor);
			}
			break;
		case GR_COMBINE_FUNCTION_SCALE_OTHER_MINUS_LOCAL_ADD_LOCAL:
			switch(factor)
			{
			case GR_COMBINE_FACTOR_OTHER_ALPHA:
				fct[num_tex] = GL_INTERPOLATE_ARB;
				source0[num_tex] = setOtherColorSource(other);
				operand0[num_tex] = GL_SRC_COLOR;
				source1[num_tex] = setLocalColorSource(local);
				operand1[num_tex] = GL_SRC_COLOR;
				source2[num_tex] = setOtherColorSource(other);
				operand2[num_tex] = GL_SRC_ALPHA;
				break;
			case GR_COMBINE_FACTOR_LOCAL_ALPHA:
				fct[num_tex] = GL_INTERPOLATE_ARB;
				source0[num_tex] = setOtherColorSource(other);
				...
				source1[num_tex] = setLocalColorSource(local);
				operand1[num_tex] = GL_SRC_COLOR;
				source2[num_tex] = setLocalColorSource(local);
				operand2[num_tex] = GL_SRC_ALPHA;
				break;
			case GR_COMBINE_FACTOR_TEXTURE_ALPHA:
				fct[num_tex] = GL_INTERPOLATE_ARB;
				...
				source1[num_tex] = setLocalColorSource(local);
				operand1[num_tex] = GL_SRC_COLOR;
				source2[num_tex] = GL_PREVIOUS_ARB;
				operand2[num_tex] = GL_SRC_ALPHA;
				break;
			case GR_COMBINE_FACTOR_TEXTURE_RGB:
				fct[num_tex] = GL_INTERPOLATE_ARB;
				...
				operand1[num_tex] = GL_SRC_COLOR;
				source2[num_tex] = GL_PREVIOUS_ARB;
				operand2[num_tex] = GL_SRC_COLOR;
				break;
			default:
				display_warning("grColorCombine(GR_COMBINE_FUNCTION_SCALE_OTHER_MINUS_LOCAL_ADD_LOCAL) : unknown factor : %x", factor);
			}
			break;
		case GR_COMBINE_FUNCTION_SCALE_MINUS_LOCAL_ADD_LOCAL:
			switch(factor)
			...
			case GR_COMBINE_FACTOR_TEXTURE_RGB:
				...
				break;
			default:
				display_warning("grColorCombine(GR_COMBINE_FUNCTION_SCALE_MINUS_LOCAL_ADD_LOCAL) : unknown factor : %x", factor);
			}
			break;
		default:
			display_warning("grColorCombine : unknown function : %x", function);
		}
		updateCombiner(num_tex);
		if (nbTextureUnits > 3)
		{
			glActiveTextureARB(GL_TEXTURE3_ARB);
			updateCombiner(num_tex+1);
		}
	}
}
               ...
               )
{
	...
	else
	{
		...
		fcta[num_tex+1] = GL_REPLACE;
		sourcea0[num_tex+1] = GL_PREVIOUS_ARB;
		operanda0[num_tex+1] = GL_SRC_ALPHA;

		switch(function)
		{
		case GR_COMBINE_FUNCTION_LOCAL:
			fcta[num_tex] = GL_REPLACE;
			sourcea0[num_tex] = setLocalAlphaSource(local);
			operanda0[num_tex] = GL_SRC_ALPHA;
			break;
		case GR_COMBINE_FUNCTION_SCALE_OTHER:
			switch (factor)
			{
			case GR_COMBINE_FACTOR_ONE:
				fcta[num_tex] = GL_REPLACE;
				sourcea0[num_tex] = setOtherAlphaSource(other);
				operanda0[num_tex] = GL_SRC_ALPHA;
				break;
			case GR_COMBINE_FACTOR_LOCAL:
				fcta[num_tex] = GL_MODULATE;
				sourcea0[num_tex] = setLocalAlphaSource(local);
				operanda0[num_tex] = GL_SRC_ALPHA;
				sourcea1[num_tex] = setOtherAlphaSource(other);
				operanda1[num_tex] = GL_SRC_ALPHA;
				break;
			case GR_COMBINE_FACTOR_ONE_MINUS_LOCAL:
				fcta[num_tex] = GL_MODULATE;
				sourcea0[num_tex] = setLocalAlphaSource(local);
				operanda0[num_tex] = GL_ONE_MINUS_SRC_ALPHA;
				sourcea1[num_tex] = setOtherAlphaSource(other);
				operanda1[num_tex] = GL_SRC_ALPHA;
				break;
			case GR_COMBINE_FACTOR_TEXTURE_ALPHA:
				fcta[num_tex] = GL_MODULATE;
				sourcea0[num_tex] = GL_PREVIOUS_ARB;
				operanda0[num_tex] = GL_SRC_ALPHA;
				sourcea1[num_tex] = setOtherAlphaSource(other);
				operanda1[num_tex] = GL_SRC_ALPHA;
				break;
			case GR_COMBINE_FACTOR_ONE_MINUS_TEXTURE_ALPHA:
				...
				break;
			default:
				display_warning("grAlphaCombine(GR_COMBINE_FUNCTION_SCALE_OTHER) : unknown factor : %x", factor);
			}
			break;
		case GR_COMBINE_FUNCTION_SCALE_OTHER_ADD_LOCAL:
			switch(factor)
			...
			case GR_COMBINE_FACTOR_TEXTURE_ALPHA:
				...
				sourcea0[num_tex] = setOtherAlphaSource(other);
				operanda0[num_tex] = GL_SRC_ALPHA;
				sourcea1[num_tex] = GL_PREVIOUS_ARB;
				operanda0[num_tex] = GL_SRC_ALPHA;
				fcta[num_tex+1] = GL_ADD;
				sourcea0[num_tex+1] = GL_PREVIOUS_ARB;
				operanda0[num_tex+1] = GL_SRC_ALPHA;
				sourcea1[num_tex+1] = setLocalAlphaSource(local);
				operanda1[num_tex+1] = GL_SRC_ALPHA;
				if (sourcea1[num_tex+1] == GL_PREVIOUS_ARB) sourcea1[num_tex+1] = GL_TEXTURE0_ARB;
				break;
			case GR_COMBINE_FACTOR_ONE_MINUS_TEXTURE_ALPHA:
				fcta[num_tex] = GL_MODULATE;
				sourcea0[num_tex] = setOtherAlphaSource(other);
			...
			case GR_COMBINE_FACTOR_LOCAL:
				...
				sourcea0[num_tex] = setOtherAlphaSource(other);
				operanda0[num_tex] = GL_SRC_ALPHA;
				sourcea1[num_tex] = setLocalAlphaSource(local);
				operanda0[num_tex] = GL_SRC_ALPHA;
				fcta[num_tex+1] = GL_ADD;
				sourcea0[num_tex+1] = GL_PREVIOUS_ARB;
				operanda0[num_tex+1] = GL_SRC_ALPHA;
			...
			case GR_COMBINE_FACTOR_ONE:
				...
				break;
			default:
				display_warning("grAlphaCombine(GR_COMBINE_FUNCTION_SCALE_OTHER_ADD_LOCAL) : unknown factor : %x", factor);
			}
			break;
		case GR_COMBINE_FUNCTION_SCALE_OTHER_MINUS_LOCAL:
			switch(factor)
			...
			case GR_COMBINE_FACTOR_TEXTURE_ALPHA:
				// this combiner mode isn't implemented correctly and doesn't work on a TNT
				fcta[num_tex] = GL_SUBTRACT_ARB;
				sourcea0[num_tex] = setOtherAlphaSource(other);
				operanda0[num_tex] = GL_SRC_ALPHA;
				sourcea1[num_tex] = setLocalAlphaSource(local);
				operanda1[num_tex] = GL_SRC_ALPHA;
				fcta[num_tex+1] = GL_MODULATE;
				sourcea0[num_tex+1] = GL_TEXTURE0_ARB;
				operanda0[num_tex+1] = GL_SRC_ALPHA;
				sourcea1[num_tex+1] = GL_PREVIOUS_ARB;
				operanda1[num_tex+1] = GL_SRC_ALPHA;
				break;
			case GR_COMBINE_FACTOR_LOCAL:
				fcta[num_tex] = GL_SUBTRACT_ARB;
				...
				operanda1[num_tex] = GL_SRC_ALPHA;
				fcta[num_tex+1] = GL_MODULATE;
				sourcea0[num_tex+1] = setLocalAlphaSource(local);
				operanda0[num_tex+1] = GL_SRC_ALPHA;
				sourcea1[num_tex+1] = GL_PREVIOUS_ARB;
				operanda1[num_tex+1] = GL_SRC_ALPHA;
				break;
			case GR_COMBINE_FACTOR_ONE:
				// doesn't work on a TNT
				fcta[num_tex] = GL_SUBTRACT_ARB;
				sourcea0[num_tex] = setOtherAlphaSource(other);
				operanda0[num_tex] = GL_SRC_ALPHA;
				sourcea1[num_tex] = setLocalAlphaSource(local);
				...
				break;
			default:
				display_warning("grAlphaCombine(GR_COMBINE_FUNCTION_SCALE_OTHER_MINUS_LOCAL) : unknown factor : %x", factor);
			}
			break;
		case GR_COMBINE_FUNCTION_SCALE_OTHER_MINUS_LOCAL_ADD_LOCAL:
			switch(factor)
			{
			case GR_COMBINE_FACTOR_TEXTURE_ALPHA:
				fcta[num_tex] = GL_INTERPOLATE_ARB;
				sourcea0[num_tex] = setOtherAlphaSource(other);
				operanda0[num_tex] = GL_SRC_ALPHA;
				sourcea1[num_tex] = setLocalAlphaSource(local);
				operanda1[num_tex] = GL_SRC_ALPHA;
				sourcea2[num_tex] = GL_PREVIOUS_ARB;
				operanda2[num_tex] = GL_SRC_ALPHA;
				break;
			default:
				display_warning("grAlphaCombine(GR_COMBINE_FUNCTION_SCALE_OTHER_MINUS_LOCAL_ADD_LOCAL) : unknown factor : %x", factor);
			}
			break;
		case GR_COMBINE_FUNCTION_SCALE_MINUS_LOCAL_ADD_LOCAL:
			switch(factor)
			...
			case GR_COMBINE_FACTOR_TEXTURE_ALPHA:
				...
				break;
			default:
				display_warning("grAlphaCombine(GR_COMBINE_FUNCTION_SCALE_MINUS_LOCAL_ADD_LOCAL) : unknown factor : %x", factor);
			}
			break;
		default:
			display_warning("grAlphaCombine : unknown function : %x", function);
		}
		updateCombinera(num_tex);
		if (nbTextureUnits > 3)
		{
			glActiveTextureARB(GL_TEXTURE3_ARB);
			updateCombinera(num_tex+1);
		}
	}
}
             ...
             )
{
	...
	{
		int num_tex;
		int will_need_lambda = 0;
		//if (rgb_invert) display_warning("grTexCombine : inverted rgb result");
		if (alpha_invert) display_warning("grTexCombine : inverted alpha result");
		...
		case GR_COMBINE_FUNCTION_ZERO:
			fct[num_tex] = GL_REPLACE;
			source0[num_tex] = GL_PRIMARY_COLOR_ARB;
			operand0[num_tex] = GL_SRC_COLOR;
			break;
		case GR_COMBINE_FUNCTION_LOCAL:
			fct[num_tex] = GL_REPLACE;
			source0[num_tex] = GL_TEXTURE;
			operand0[num_tex] = GL_SRC_COLOR;
			break;
		case GR_COMBINE_FUNCTION_SCALE_OTHER:
			switch(rgb_factor)
			...
			case GR_COMBINE_FACTOR_ONE:
				...
				break;
			default:
				display_warning("grTextCombine(GR_COMBINE_FUNCTION_SCALE_OTHER) : unknown rgb factor : %x", rgb_factor);
			}
			break;
		case GR_COMBINE_FUNCTION_SCALE_OTHER_ADD_LOCAL:
			switch(rgb_factor)
			...
			case GR_COMBINE_FACTOR_OTHER_ALPHA:
				...
				{
					fct[num_tex-1] = GL_MODULATE;
					source0[num_tex-1] = GL_TEXTURE;
					operand0[num_tex-1] = GL_SRC_COLOR;
					source1[num_tex-1] = GL_TEXTURE;
					operand1[num_tex-1] = GL_SRC_ALPHA;
					fct[num_tex] = GL_ADD;
					source0[num_tex] = GL_PREVIOUS_ARB;
					operand0[num_tex] = GL_SRC_COLOR;
					source1[num_tex] = GL_TEXTURE;
			...
			case GR_COMBINE_FACTOR_DETAIL_FACTOR:
				if (num_tex==1)
				{
					...
					source0[num_tex-1] = GL_TEXTURE;
					operand0[num_tex-1] = GL_SRC_COLOR;
					source1[num_tex-1] = GL_CONSTANT_ARB;
					operand1[num_tex-1] = GL_SRC_ALPHA;
					fct[num_tex] = GL_ADD;
					source0[num_tex] = GL_PREVIOUS_ARB;
					operand0[num_tex] = GL_SRC_COLOR;
				...
				break;
			default:
				display_warning("grTextCombine(GR_COMBINE_FUNCTION_SCALE_OTHER_ADD_LOCAL) : unknown rgb factor : %x", rgb_factor);
			}
			break;
		case GR_COMBINE_FUNCTION_SCALE_OTHER_MINUS_LOCAL_ADD_LOCAL:
			switch(rgb_factor)
			...
			case GR_COMBINE_FACTOR_DETAIL_FACTOR:
				...
				source1[num_tex] = GL_TEXTURE;
				operand1[num_tex] = GL_SRC_COLOR;
				source2[num_tex] = GL_CONSTANT_ARB;
				operand2[num_tex] = GL_SRC_ALPHA;
				will_need_lambda = 1;
				break;
			case GR_COMBINE_FACTOR_ONE_MINUS_LOCAL:
				...
				source1[num_tex] = GL_TEXTURE;
				operand1[num_tex] = GL_SRC_COLOR;
				source2[num_tex] = GL_TEXTURE;
				operand2[num_tex] = GL_ONE_MINUS_SRC_COLOR;
				break;
			default:
				display_warning("grTextCombine(GR_COMBINE_FUNCTION_SCALE_OTHER_MINUS_LOCAL_ADD_LOCAL) : unknown rgb factor : %x", rgb_factor);
			}
			break;
		case GR_COMBINE_FUNCTION_SCALE_OTHER_MINUS_LOCAL:
			switch(rgb_factor)
			...
			case GR_COMBINE_FACTOR_LOCAL:
				...
				if (num_tex==1)
				{
					fct[num_tex-1] = GL_SUBTRACT_ARB;
					source0[num_tex-1] = GL_TEXTURE0_ARB;
					operand0[num_tex-1] = GL_SRC_COLOR;
					source1[num_tex-1] = GL_TEXTURE1_ARB;
					operand1[num_tex-1] = GL_SRC_COLOR;
					fct[num_tex] = GL_MODULATE;
					source0[num_tex] = GL_PREVIOUS_ARB;
					operand0[num_tex] = GL_SRC_COLOR;
				...
				break;
			default:
				display_warning("grTextCombine(GR_COMBINE_FUNCTION_SCALE_OTHER_MINUS_LOCAL) : unknown rgb factor : %x", rgb_factor);
			}
		default:
			display_warning("grTextCombine : unknown rgb function : %x", rgb_function);
		}
		...
		case GR_COMBINE_FUNCTION_ZERO:
			fcta[num_tex] = GL_REPLACE;
			sourcea0[num_tex] = GL_PRIMARY_COLOR_ARB;
			operanda0[num_tex] = GL_SRC_ALPHA;
			break;
		case GR_COMBINE_FUNCTION_LOCAL:
			fcta[num_tex] = GL_REPLACE;
			sourcea0[num_tex] = GL_TEXTURE;
			operanda0[num_tex] = GL_SRC_ALPHA;
			break;
		case GR_COMBINE_FUNCTION_SCALE_OTHER:
			switch(alpha_factor)
			...
			case GR_COMBINE_FACTOR_ONE:
				...
				break;
			default:
				display_warning("grTextCombine(GR_COMBINE_FUNCTION_SCALE_OTHER) : unknown alpha factor : %x", alpha_factor);
			}
			break;
		case GR_COMBINE_FUNCTION_SCALE_OTHER_ADD_LOCAL:
			switch(alpha_factor)
			...
			case GR_COMBINE_FACTOR_DETAIL_FACTOR:
				if (num_tex==1)
				{
					fct[num_tex-1] = GL_MODULATE;
					source0[num_tex-1] = GL_TEXTURE;
					operand0[num_tex-1] = GL_SRC_ALPHA;
					source1[num_tex-1] = GL_CONSTANT_ARB;
					operand1[num_tex-1] = GL_SRC_ALPHA;
					fct[num_tex] = GL_ADD;
					source0[num_tex] = GL_PREVIOUS_ARB;
				...
				break;
			default:
				display_warning("grTextCombine(GR_COMBINE_FUNCTION_SCALE_OTHER_ADD_LOCAL) : unknown alpha factor : %x", alpha_factor);
			}
			break;
		case GR_COMBINE_FUNCTION_SCALE_OTHER_MINUS_LOCAL_ADD_LOCAL:
			switch(alpha_factor)
			...
			case GR_COMBINE_FACTOR_DETAIL_FACTOR:
				...
				sourcea1[num_tex] = GL_TEXTURE;
				operanda1[num_tex] = GL_SRC_ALPHA;
				sourcea2[num_tex] = GL_CONSTANT_ARB;
				operanda2[num_tex] = GL_SRC_ALPHA;
				will_need_lambda = 1;
				break;
			default:
				display_warning("grTextCombine(GR_COMBINE_FUNCTION_SCALE_OTHER_MINUS_LOCAL_ADD_LOCAL) : unknown alpha factor : %x", alpha_factor);
			}
			break;
		case GR_COMBINE_FUNCTION_SCALE_MINUS_LOCAL_ADD_LOCAL:
			switch(alpha_factor)
			...
			case GR_COMBINE_FACTOR_DETAIL_FACTOR:
				fcta[num_tex] = GL_MODULATE;
				sourcea0[num_tex] = GL_CONSTANT_ARB;
				operanda0[num_tex] = GL_ONE_MINUS_SRC_ALPHA;
				sourcea1[num_tex] = GL_TEXTURE;
				operanda1[num_tex] = GL_SRC_ALPHA;
				will_need_lambda = 1;
				break;
			default:
				display_warning("grTextCombine(GR_COMBINE_FUNCTION_SCALE_MINUS_LOCAL_ADD_LOCAL) : unknown alpha factor : %x", alpha_factor);
			}
			break;
		case GR_COMBINE_FUNCTION_SCALE_MINUS_LOCAL_ADD_LOCAL_ALPHA:
			switch(alpha_factor)
			...
			case GR_COMBINE_FACTOR_OTHER_ALPHA:
				...
				break;
			default:
				display_warning("grTextCombine(GR_COMBINE_FUNCTION_SCALE_MINUS_LOCAL_ADD_LOCAL_ALPHA) : unknown alpha factor : %x", alpha_factor);
			}
			break;
		default:
			display_warning("grTextCombine : unknown alpha function : %x", alpha_function);
		...
		if (will_need_lambda != need_lambda[num_tex])
		{
			need_lambda[num_tex] = will_need_lambda;
			if (need_lambda[num_tex])
			{
				int i;
				for (i=0; i<3; i++) lambda_color[num_tex][i] = texture_env_color[i];
				lambda_color[num_tex][3] = lambda;
				glTexEnvfv(GL_TEXTURE_ENV, GL_TEXTURE_ENV_COLOR, lambda_color[num_tex]);
			}
			else
				glTexEnvfv(GL_TEXTURE_ENV, GL_TEXTURE_ENV_COLOR, texture_env_color);
		}
...
grFogMode( GrFogMode_t mode )
{
	...
	if(glsl_support) need_to_compile = 1;
}
                    ...
                    float nearZ, float farZ )
{
	...
	if(!glsl_support)
	{
		...
		glFogi(GL_FOG_COORDINATE_SOURCE_EXT, GL_FOG_COORDINATE_EXT);
		glFogf(GL_FOG_START, nearZ /*/ 256.0f*/);
		glFogf(GL_FOG_END, farZ /*/ 256.0f*/);
	}
	else
	{
		/*int near_location;
		...
		near_location = glGetUniformLocationARB(program_object, "near");
		glUniform1fARB(near_location, nearF);

		far_location = glGetUniformLocationARB(program_object, "far");
		glUniform1fARB(far_location, farF);*/
...
grChromakeyMode( GrChromakeyMode_t mode )
{
	...
	case GR_CHROMAKEY_ENABLE:
		if(!glsl_support) display_warning("grChromakeyMode : unknown mode : %x", mode);
		else chroma_enabled = 1;
		break;
	default:
...
static void setPattern()
{
	...
	if(!glsl_support) glPolygonStipple(stip);
	else
	{
...
grStippleMode( GrStippleMode_t mode )
{
	...
	case GR_STIPPLE_DISABLE:
		if(!glsl_support) glDisable(GL_POLYGON_STIPPLE);
		else
		{
			dither_enabled = 0;
			glActiveTextureARB(GL_TEXTURE2_ARB);
